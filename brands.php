...
$brandId = $_GET['id']; 
$brandName = $_GET['name'];
getBrand($link , $brandId , $brandName );

function getBrand($conn,$brandId , $brandName){
    echo'<section id="products" class="products mt-5">  
        <div class=" text-center text-uppercase mt-5">
            <h2 class="section-head">'.$brandName.'</h2>
        </div>                      
        <div class = "d-flex justify-content-around mt-2 flex-wrap">';
            $select_data = "SELECT * FROM `product`  LEFT JOIN `brands` ON product.pro_brand = brands.b_id WHERE pro_brand = $brandId  LIMIT 16";
...
include 'footer.php';        
                 ...
                 echo '<p class = "text-danger">This Brand is not found</P>';
                 ?>
                        <!-- bootstrap js -->
