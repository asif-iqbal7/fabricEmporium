...
<section id="products" class="products mt-5">  
    <h2 class="section-head text-center text-uppercase mt-2"><a href="#">All Products</a></h2>                    
  <div class = "d-flex justify-content-around mt-2 flex-wrap">
        <?php
        $select_allProducts="SELECT * FROM `product` LEFT JOIN `brands` ON product.pro_brand = brands.b_id LIMIT 28";
                $result_allProducts=mysqli_query($link,$select_allProducts);
                    while($row_data=mysqli_fetch_assoc($result_allProducts)){
