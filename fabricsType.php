...
// Fabrics Type Page
$fabricsId = $_GET['id']; 
$fabricsName = $_GET['name'];

getFabrics($link , $fabricsId , $fabricsName );

function getFabrics($conn,$fabric_id,$fabrics_name){
    echo'<section id="products" class="products mt-5">  
        <div class=" text-center text-uppercase mt-5">
            <h2 class="section-head">'.$fabrics_name.'</h2>
        </div>                   
        <div class = "d-flex justify-content-around mt-2 flex-wrap">';   
            $select_data ="SELECT * FROM `product`  LEFT JOIN `brands` ON product.pro_brand = brands.b_id WHERE pro_fab_type = $fabric_id  LIMIT 16"; 
...
include 'footer.php'; 
                ...
                echo '<p class = "text-danger">This fabrics type is not found</P>';
                    ?>
            <!-- bootstrap js -->
